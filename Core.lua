...
innerBinds = {}
innerBinds["HAP"] = CreateFrame("Frame", "HAP", UIParent)
local TCONRO = nil
local s_count = 0
...
function _CBLD.loadInnerTarget()
    innerBinds["target"] = {}
    LoadBindings(2)
    innerBinds["target"][41] =
        CreateFrame("Button", "innerTarget" .. 41, innerBinds["HAP"], "SecureActionButtonTemplate")
    innerBinds["target"][41]:SetAttribute("type", "target")
    innerBinds["target"][41]:SetAttribute("unit", "player")
    --print(SetBindingClick("F1", innerBinds["target"][41]:GetName(), "LeftButton"))
    SaveBindings(2)

    for Index = 42, 45 do
        innerBinds["target"][Index] =
            CreateFrame("Button", "innerTarget" .. Index, innerBinds["HAP"], "SecureActionButtonTemplate")
        innerBinds["target"][Index]:SetAttribute("type", "target")
        innerBinds["target"][Index]:SetAttribute("unit", "party" .. (Index - 41))
        SetBindingClick("F" .. (Index - 40), innerBinds["target"][Index]:GetName(), "LeftButton")
        SaveBindings(2)
    end
    ...
    for Index = 1, 10 do
        innerBinds["target"][Index] =
            CreateFrame("Button", "innerTarget" .. Index, innerBinds["HAP"], "SecureActionButtonTemplate")
        innerBinds["target"][Index]:SetAttribute("type", "target")
        innerBinds["target"][Index]:SetAttribute("unit", "raid" .. Index)
        SetBindingClick("NUMPAD" .. (Index - 1), innerBinds["target"][Index]:GetName(), "LeftButton")
        SaveBindings(2)
    end
    for Index = 11, 20 do
        innerBinds["target"][Index] =
        ...
        innerBinds["target"][Index]:SetAttribute("type", "target")
        innerBinds["target"][Index]:SetAttribute("unit", "raid" .. Index)
        SetBindingClick("CTRL-NUMPAD" .. (Index - 1) % 10, innerBinds["target"][Index]:GetName(), "LeftButton")
        SaveBindings(2)
    end
    for Index = 21, 30 do
        innerBinds["target"][Index] =
        ...
        innerBinds["target"][Index]:SetAttribute("type", "target")
        innerBinds["target"][Index]:SetAttribute("unit", "raid" .. Index)
        SetBindingClick("ALT-NUMPAD" .. (Index - 1) % 10, innerBinds["target"][Index]:GetName(), "LeftButton")
        SaveBindings(2)
    end
    for Index = 31, 40 do
        innerBinds["target"][Index] =
        ...
        innerBinds["target"][Index]:SetAttribute("type", "target")
        innerBinds["target"][Index]:SetAttribute("unit", "raid" .. Index)
        SetBindingClick("CTRL-ALT-NUMPAD" .. (Index - 1) % 10, innerBinds["target"][Index]:GetName(), "LeftButton")
        SaveBindings(2)
    end
end
...
function auraexists(unit, auraName)
    ...
    isCastByPlayer,
    nameplateShowAll,
    timeMod = AuraUtil.FindAuraByName(auraName, unit, "player")
...
function interruptable(unit)
    ...
    local cname, ctext, ctexture, cstartTimeMS, cendTimeMS, cisTradeSkill, cnotInterruptible, cspellId =
...
function lowest()
    --print(1 / tonumber(string.format("%.2f", 35)))
    ...
    if name ~= nil then
        --print(name .. "Is The Lowest Health " .. string.format("%.2f", lowesthp))
    end
    ...
    elseif string.find(lowestUnit, "party") then
        ...
        local getColorModifier = string.format("%.2f", 1 / tonumber(getID))
    ...
    elseif string.find(lowestUnit, "raid") then
        ...
        local getID2 = string.sub(lowestUnit, 6, 6)
        local getColorModifier = string.format("%.2f", 1 / tonumber(getID))
        local getColorModifier2 = "0"
        if getID2 ~= "" then
            getColorModifier2 = string.format("%.2f", 1 / tonumber(getID2))
        end
        CrazyFrame[41].t:SetColorTexture(1, getColorModifier, getColorModifier2, 1)
    else
        CrazyFrame[41].t:SetColorTexture(0, 0, 0, 1)
    end
    return lowestUnit
...
function _CBLD.ParseMacro(macroId, slot)
    local macrobody = GetMacroBody(macroId)
    local macroName
    if not macrobody then
        macroName = GetActionText(slot)
        if macroName then
            --print("Getting Macro Body For Macro Name => " .. macroName)
            macrobody = GetMacroBody(macroName)
            if not macrobody then
                return nil
            end
        else
    ...
    local function IsValidSpell(spellName)
    ...
    if mcb ~= "" then
        ...
        if spellName then
            if mcbtype == "spell" then
                ...
                return IsValidSpell(spellid) and macroName, spellid, mcbtype or nil
            end
            if mcbtype == "item" then
                local itemID, itemType, itemSubType, itemEquipLoc, icon, itemClassID, itemSubClassID =
                    GetItemInfoInstant(spellName)
                return IsValidItem(itemID) and macroName, itemID, mcbtype or nil
            end
        end
    end
...
function _CBLD.ParseLink(hyperlink)
    local color, linkType, linkData, text =
        string.match(hyperlink, "|?c?f?f?(%x*)|?H?([^:]*):?(%d+)|?h?%[?([^%[%]]*)%]?|?h?|?r?")
    return color, linkType, linkData, text
end

function _CBLD.GetMacroIndexByTexture(texture)
    for i = 1, GetNumMacros() do
        local _, macroTexture, _ = GetMacroInfo(i)
        -- Compare macro texture with the target texture
        if macroTexture == texture then
            return i
        end
    end
end
...
---[[
    local idInt = tonumber(spellId)
    if idInt then
        if not keyBind and BindingTable.id[idInt] then
            print("already added")
            return
        end
    end
   ---[[
    ...
    if actionType == "spell" then
        ...
        if id then
            if not BindingTable.id[id] then
                ...
                BindingTable.name[id] = spellName
    ...
    elseif actionType == "macro" then
        ...
        if id then
            --local at = bt.icon:GetTexture()
            --local index = _CBLD.GetMacroIndexByTexture(at)
            local macroName, spell, mcbtype = _CBLD.ParseMacro(id, slot)
            if spell then
                ...
                if tid then
                    if not BindingTable.id[tid] then
						...
						BindingTable.name[id] = macroName
                        ...
                        --print("Binding Macro -> " .. BindingTable.spellType[tid] .. " : " .. BindingTable.id[tid] .. " : " .. BindingTable.bind[tid] )
                    end
                end
            end
        end
...
local function printRed(message)
    ChatFrame1:AddMessage("|cFFFF0000" .. message .. "|r")
end
function checkDuplicatesMacros()
    local global, perChar = GetNumMacros()
    local macroNames = {} -- Table to store encountered macro names
    -- Check character-specific macros
    for i = 121, 121 + perChar - 1 do
        local name, icon, body = GetMacroInfo(i)
        if name then
            if macroNames[name] then
                printRed("Duplicate macro name found:" .. name)
            else
...
function _CBLD.BindSnap(checkDuplicates)
    ...
    if checkDuplicates then
        checkDuplicatesMacros()
    end

    local function addBinding(actionType, name, key, spellId, slot)
        if actionType and name and key then
            if (bonusBar == 0 and slot < 73) or
                (bonusBar == 1 and slot > 12 and slot < 85) or
                (bonusBar == 2 and slot > 12 and (slot < 73 or (slot > 84 and slot < 97))) or
                (bonusBar == 3 and slot > 12 and (slot < 73 or (slot > 84 and slot < 109))) or
                (bonusBar == 4 and slot > 12 and (slot < 73 or slot > 108)) then
                _CBLD.AddBind(actionType, name, key, spellId, slot)
            end
        end
    end

    local function checkAndAddBinding(spellName, replacementSpellId)
        if spellTracker[spellName] then
            printRed("Duplicated Spell Found: " .. spellName)
        else
            spellTracker[spellName] = true
            return replacementSpellId
        end
    end

    for slot = 1, 120 do
        local name = _G["Bartender4"] and "CLICK BT4Button " .. slot .. ":LeftButton" or
            (slot <= 24 or slot > 72) and "ACTIONBUTTON" .. ((slot - 1) % 12 + 1) or
            (slot <= 36) and "MULTIACTIONBAR3BUTTON" .. (slot - 24) or
            (slot <= 48) and "MULTIACTIONBAR4BUTTON" .. (slot - 36) or
            (slot <= 60) and "MULTIACTIONBAR2BUTTON" .. (slot - 48) or
            "MULTIACTIONBAR1BUTTON" .. (slot - 60)

        local key = name and API_GetBindingKey(name)

        if key and strlen(key) > 4 then
            -- Shorten keybinding names
            key = strupper(gsub(gsub(gsub(gsub(gsub(gsub(gsub(gsub(gsub(gsub(
                        key, "%s+", ""), "ALT", "A"), "CTRL", "C"), "SHIFT", "S"),
                    "--", "-"), "NUMPAD", "N"), "PLUS", "+"), "MINUS", "-"),
                "MULTIPLY", "*"), "DIVIDE", "/"))

            key = string.sub(key, 2, 3) == "--" and string.sub(key, 1, 1) .. "-" or gsub(key, "-", "")
        end

        local actionType, spellId, _ = API_GetActionInfo(slot)
        local spellIdAlter = nil
        if spellId then
            local spellName, _, _, _, _, _, _, _ = GetSpellInfo(spellId)
            if spellName == "Sentinel" then
                spellId = checkAndAddBinding(spellName, 31884)
            elseif spellName == "Summon Felguard" then
                spellId = checkAndAddBinding(spellName, 30146)
            elseif spellName == "Call Dreadstalkers" then
                spellId = checkAndAddBinding(spellName, 418543)
                spellIdAlter = 104316
            end

            addBinding(actionType, name, key, spellId, slot)

            if spellIdAlter ~= nil then
                addBinding(actionType, name, key, spellIdAlter, slot)
            end
        end
    end
end
...
hiding:RegisterEvent("CURRENT_SPELL_CAST_CHANGED")
...
local function hideBG(self, event, ...)
    ...
    elseif event == "CURRENT_SPELL_CAST_CHANGED" then
        if _CBLD.IsSpellTargeting() == true then
            print("Awaits Target")
            CrazyFrame[23].t:SetColorTexture(1, 0.0, 0.0)
        else
            -- print("Immediate Spell")
            CrazyFrame[23].t:SetColorTexture(0.0, 0.0, 0.0)
        end
    end
end
...
function _CBLD.PushCurrentSpellFromId(index, itemid, keybind)
    ...
    local cost = 0
    local costTable = GetSpellPowerCost(itemid)
    local powerType = nil
    if (costTable ~= nil) then
        for key, costInfo in pairs(costTable) do
            cost = costInfo.cost
            powerType = costInfo.type
            break
        end
    end
    --print("Cost " .. cost)
    local upower = UnitPower("player", powerType)
    --print("upower " .. upower)
    if upower < cost then
        CrazyFrame[index].t:SetColorTexture(0.0, 0.0, 0.0)
        return
...
function _CBLD.WeekSpell()
    if WeakAuras.GetRegion("KS") == nil then
        CrazyFrame[22].t:SetColorTexture(0.0, 0.0, 0.0)
        return nil
    end
    CrazyFrame[22].t:SetColorTexture(0.0, 0.0, 0.0)
...
function _CBLD.IsSpellTargeting()
...
function _CBLD.PushDefaultData2Screen()
    ...
    if (exists == false) then
        CrazyFrame[12].t:SetColorTexture(1.0, 0.0, 0.0)
    else
        CrazyFrame[12].t:SetColorTexture(0.0, 0.0, 0.0)
    end
    -- если ткрыт чат бар
    if ACTIVE_CHAT_EDIT_BOX then
        CrazyFrame[13].t:SetColorTexture(1.0, 0.0, 0.0)
    else
    ...
    -- если верхом
    if IsMounted() then
        CrazyFrame[14].t:SetColorTexture(1.0, 0.0, 0.0)
    else
    ...
    if UnitIsDead("target") then
        CrazyFrame[15].t:SetColorTexture(1.0, 0.0, 0.0)
    else
    ...
    if UnitIsFriend("player", "target") then
        CrazyFrame[16].t:SetColorTexture(1.0, 0.0, 0.0)
    else
    ...
    if InCombatLockdown() == false then
        CrazyFrame[18].t:SetColorTexture(1.0, 0.0, 0.0)
    else
    ...
    if UnitCanAttack("player", "target") == true then
        ...
        CrazyFrame[19].t:SetColorTexture(1.0, 0.0, 0.0)
    end

    if _CBLD.IsPlayerCst() == true then
        --print("cast")
        CrazyFrame[20].t:SetColorTexture(1.0, 0.0, 0.0)
    else
        --print("no cast")
        CrazyFrame[20].t:SetColorTexture(0.0, 0.0, 0.0)
    end
    ...
    if _CBLD.IsPlayerChn() == true then
        --print("chn")
        CrazyFrame[21].t:SetColorTexture(1.0, 0.0, 0.0)
    else
        --print("no cn")
...
function OnUIErrorMessage(self, event, messageType, message)
    ...
    if errorName then
        if
            errorName == "ERR_SPELL_OUT_OF_RANGE" or errorName == "ERR_BADATTACKFACING" or
            errorName == "ERR_SPELL_FAILED_S"
        then
